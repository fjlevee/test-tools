...
public class HttpResponseParser
{
    ...
    private HttpResponseParser() {
      this.logger = Logger
          .getLogger(com.fjl.sip.test.parser.http.HttpResponseParser.class);
    }
     ...
     * 
     */
    public ApplicationObject parseHttpResponse(String httpResponseContent) throws SipClientParserException {
      try {
        if (logger.isDebugEnabled())
          logger
              .debug("HttpResponseParser.parseHttpResponse: Start...");
        // Instanciate the XML parser
        DocumentBuilder parser = DocumentBuilderFactory.newInstance().newDocumentBuilder();
        Document document = parser.parse( new InputSource( new StringReader( httpResponseContent)));
