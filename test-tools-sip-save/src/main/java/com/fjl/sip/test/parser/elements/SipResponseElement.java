...
public class SipResponseElement extends SipTestCallFlowAction
{
   ...
   */
  @Override
  public boolean parseSipTestCallFlowAction ( SipTestCallFlow sipTestCallFlow, org.w3c.dom.Node siptestCallFlowAction,
      int sipTestActionId) throws SipClientParserException
  {
    if (logger.isTraceEnabled()) logger.trace( "SipResponseElement.parseSipTestCallFlowAction Start");
    this.sipTestActionId = sipTestActionId;
    // Get the siptestCallFlowAction attributes
    org.w3c.dom.NamedNodeMap sipTestCallFlowNodesAttributes = siptestCallFlowAction.getAttributes();
    // Auto attribute
    try
    {
    ...
    try
    {
      String codeString = sipTestCallFlowNodesAttributes.getNamedItem( SipTestCallFlowUtils.CODE_ATTRIBUTE)
          .getNodeValue();
      if (codeString != null)
      {
    ...
    try
    {
      callid = sipTestCallFlowNodesAttributes.getNamedItem( SipTestCallFlowUtils.CALL_ID_ATTRIBUTE).getNodeValue();
    }
    catch (NullPointerException e)
    {
      throw new SipClientParserException(
    ...
    if (direction == ActionDirection.TO_SERVER)
    {
      ...
      // response received by the B2B Component search if the to Uri actor has been created yet;
      ...
      if (callFlowActor == null)
      {
        ...
        if (XmlElementsConfiguration.getActorForId( actorId) != null)
        {
          if (logger.isDebugEnabled())
            logger
                .debug( "SipResponseElement.parseSipTestCallFlowAction: this response is an internal network request => Don't add it to the list of actions");
          return false;
        }
    ...
    if (logger.isTraceEnabled()) logger.trace( "SipResponseElement.parseSipTestCallFlowAction End");
    return true;
  }
   ...
   * 
   * @param method specifies the new Sip Response method
   */
   ...
   * 
   * @param code specifies the new Sip Response code
   */
   ...
   * 
   * @param callid specifies the new Sip Response callid
   */
   ...
   * 
   * @param fromSipUri specifies the new Sip Response From sip uri
   */
   ...
   * 
   * @param toSipUri specifies the new Sip Response To sip uri
   */
  ...
  {
    return "SipResponse, Method: " + method + ", Status: " + code + ", Direction: " + direction;
  }
