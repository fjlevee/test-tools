...
public class WaitElement extends SipTestCallFlowAction
{
  ...
  {
    if (logger.isTraceEnabled()) logger.trace( "WaitElement.parseSipTestCallFlowAction Start");
    ...
    try
    {
      String timeToWaitString = sipTestCallFlowNodesAttributes.getNamedItem( SipTestCallFlowUtils.TIME_TO_WAIT_ATTRIBUTE)
          .getNodeValue();
      this.timeToWait = Long.parseLong( timeToWaitString);
      if (logger.isTraceEnabled()) logger.trace( "WaitElement.parseSipTestCallFlowAction Time To Wait is:" +timeToWait+" ms.");
    }
    ...
    if (logger.isTraceEnabled()) logger.trace( "WaitElement.parseSipTestCallFlowAction End");
    return true;
  }
