...
import com.fjl.test.tools.configuration.SipTestProfileConfiguration;
import com.fjl.test.tools.configuration.TestProfileConfigurationException;
import com.fjl.test.tools.configuration.TestProfilesManager;
import com.fjl.test.tools.files.StartExtensionFileFilter;
...
/**
 * 
 * This class SipTestProfilesManager is reponsible for loading and created all
 * sip server profiles configuration files
 * 
 * @author FJ LEVEE
 * 
 * @version G0R0C1
 * @since G0R0C1
 */
public class TestProfilesManager {
    /**
     * Singleton
     */
    private static TestProfilesManager instance;
     ...
     */
    private TestProfilesManager() throws TestProfileConfigurationException {
        ...
        configFolderPath = FJLToolsConfiguration.getFJLToolsConfigurationPath()
                + configFolderPath;
        ...
        if (!configFolder.exists()) {
            throw new TestProfileConfigurationException(
                    "SipTestProfilesManager constructor error: Can not find a folder with path: "
                            + configFolderPath);
        }
        FileFilter filter = new StartExtensionFileFilter("TestProfile_",
                new String[] { ".xml" });

        File[] serversConfigurationsFiles = configFolder.listFiles(filter);
        if (serversConfigurationsFiles == null) {
            throw new TestProfileConfigurationException(
                    "SipTestProfilesManager constructor error: Can not find server configuration file in folder: "
                            + configFolderPath);
        }
        for (int i = 0; i < serversConfigurationsFiles.length; i++) {
            SipTestProfileConfiguration serverConfiguration = new SipTestProfileConfiguration(
                    serversConfigurationsFiles[i].getPath());
            sipTestProfileConfigurations.put(
                    serverConfiguration.getSipServerConfigurationName(),
                    serverConfiguration);
     ...
     * @throws TestProfileConfigurationException
     */
    public static SipTestProfileConfiguration getSipServerProfileConfiguration(
            String configurationName) throws TestProfileConfigurationException {
        if (instance == null) {
            instance = new TestProfilesManager();
        }
