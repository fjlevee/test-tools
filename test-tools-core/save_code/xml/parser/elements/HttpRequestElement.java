...
public class HttpRequestElement extends SipTestCallFlowAction {
    ...
    public boolean parseSipTestCallFlowAction(SipTestCallFlow sipTestCallFlow,
        ...
        if (logger.isTraceEnabled())
            logger.trace("HttpRequestAction.parseSipTestCallFlowAction Start");
        ...
        // Http request method
        try {
            String method = SipTestCallFlowNodesAttributes.getNamedItem(
        ...
        SipTestCallFlowActor httpClientActor = sipTestCallFlow
                .getHttpClientSipTestCallFlowActor();
        // Checks if the Http Client has been created
        if (httpClientActor == null) {
            // not created => create it
            httpClientActor = new SipTestCallFlowActor(
                    SipTestCallFlow.HTTP_CLIENT_ACTOR_NAME,
        ...
        setSipTestCallFlowActor(sipTestCallFlow
                .getHttpClientSipTestCallFlowActor());
        // Add this HttpRequest Action to the HttpClient Actor
        httpClientActor.addSipTestCallFlowAction(this);
        if (logger.isTraceEnabled())
            logger.trace("HttpRequestAction.parseSipTestCallFlowAction End");
        return true;
    }
