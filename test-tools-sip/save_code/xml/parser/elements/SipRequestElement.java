...
public class SipRequestElement extends SipTestCallFlowAction {
     ...
     * server. The request uri is a server request uri. This parameter is set to
     * true if the last contact uri received in a Initial INVITE request or in a
    ...
    public boolean parseSipTestCallFlowAction(SipTestCallFlow sipTestCallFlow,
        ...
        if (logger.isTraceEnabled())
            logger.trace("SipRequestElement.parseSipTestCallFlowAction Start");
        ...
        try {
            String authenticatedString = SipTestCallFlowNodesAttributes
                    .getNamedItem(SipTestCallFlowUtils.AUTHENTICATED_ATTRIBUTE)
                    .getNodeValue();
        ...
        // Sip request ifctype
        try {
            String ifctype = SipTestCallFlowNodesAttributes.getNamedItem(
                    SipTestCallFlowUtils.IFC_TYPE_ATTRIBUTE).getNodeValue();
        ...
        // Sip Request Call ID
        try {
            String callid = SipTestCallFlowNodesAttributes.getNamedItem(
        ...
        finalDestinationIsServer = false;
        try {
            String value = SipTestCallFlowNodesAttributes.getNamedItem(
                    SipTestCallFlowUtils.FINAL_DESTINATION_IS_SERVER)
                    .getNodeValue();
        ...
        if (direction == ActionDirection.TO_SERVER) {
            ...
            // request received by the B2B Component search if the from Uri
            // actor has been created yet;
            ...
            if (callFlowActor == null) {
                ...
                if (XmlElementsConfiguration.getActorForId(actorId) != null) {
                    if (logger.isDebugEnabled())
                        logger.debug("SipRequestElement.parseSipTestCallFlowElement: this "
                                + method
                                + " request is an internal network request => Don't add it to the list of actions");
                    return false;
                }
            ...
            String actorReceiverId = SipTestCallFlowUtils
                    .getUserIdentifier(toSipUri);
            SipTestCallFlowActor callFlowActorReceiver = sipTestCallFlow
                    .getSipTestCallFlowActor(actorReceiverId);
            ...
            if (callFlowActorReceiver == null) {
                if (finalDestinationIsServer) {
                    ...
                    if (logger.isDebugEnabled())
                        logger.debug("SipRequestElement.parseSipTestCallFlowElement: this  "
                                + method
                                + " request is destinated to a server: check if the sender is a client");
                    ...
                    if (callFlowActor.isSipClient()) {
                        ...
                        if (logger.isDebugEnabled())
                            logger.debug("SipRequestElement.parseSipTestCallFlowElement: this  "
                                    + method
                                    + " request is destinated to a server and sent by a client");
                    } else {
                        if (logger.isDebugEnabled())
                            logger.debug("SipRequestElement.parseSipTestCallFlowElement: this  "
                                    + method
                    ...
                    // Check if the request is not destinated to a network
                    // component
                    if (XmlElementsConfiguration.getActorForId(actorId) != null) {

                        if (logger.isDebugEnabled())
                            logger.debug("SipRequestElement.parseSipTestCallFlowElement: this request is an internal network request => Don't add it to the list of actions");
                        return false;

                    }

                    callFlowActorReceiver = new SipTestCallFlowActor(
                            actorReceiverId,
                            sipTestCallFlow.getNextCurrentUEActorId(),
                            toSipUri, true);
                    sipTestCallFlow
                            .addSipTestCallFlowActor(callFlowActorReceiver);
            ...
            // request sent by the B2B Component search if the to Uri actor has
            // been created yet;
        ...
        if (logger.isTraceEnabled())
            logger.trace("SipRequestElement.parseSipTestCallFlowAction End");
        return true;
    }
     ...
     * 
     * @param method
     *            specifies the new sip request method
     */
     ...
     * 
     * @param ifctype
     *            specifies the new sip request ifc type
     */
     ...
     * 
     * @param callid
     *            specifies the new sip request callid
     */
     ...
     * 
     * @param fromSipUri
     *            specifies the new sip request From sip uri
     */
     ...
     * 
     * @param toSipUri
     *            specifies the new sip request To sip uri
     */
    ...
    /**
     * Get the message actor receiver in case of this Request is an initial SIP
     * Request
     * 
     * @return the message actor receiver in case of this Request is an initial
     *         SIP Request
     */
    ...
    /**
     * Set the message actor receiver in case of this Request is an initial SIP
     * Request
     * 
     * @param sipTestCallFlowActorReceiver
     */
    public void setSipTestCallFlowActorReceiver(
            SipTestCallFlowActor sipTestCallFlowActorReceiver) {
        this.sipTestCallFlowActorReceiver = sipTestCallFlowActorReceiver;
    }

    /**
     * Get if the request final destination is a server. TheGet if the request
     * uri is a server request uri. This parameter is set to true if the last
     * contact uri received in a Initial INVITE request or in a Initial INVITE
     * final response has to be used to send this request. Today we use it only
