...
public class SipClientUAEvent {
    ...
    private Logger logger = LoggerFactory
            .getLogger(com.fjl.test.tools.sip.SipClientUAEvent.class);
    /**
     * Call ID
     */
    private String callId;
    /**
     * Method Can be a Sip Method (for Request or response method) or a
     * TIME_OUT_EVENT
     */
     ...
     * @param callId
     * @param Specifies
     *            the Call ID on which the Event Has occured
     * @param method
     *            Specifes the Method. Can be a Sip Method (for Request or
     *            response method) or a TIME_OUT_EVENT
     * @param eventClass
     ...
     *            Must be set to -1 in case of the Event awaited is not a
     *            ResponseEvent
     * @return
     */
    public boolean checkEventParameters(String callId, String method,
            Class eventClass, int statusCode) {
        if (eventClass != ResponseEvent.class) {
            return ((this.callId.equals(callId))
                    && (this.method.equals(method)) && (this.event.getClass() == eventClass));
        } else {
            return ((this.callId.equals(callId))
                    && (this.method.equals(method))
                    && (this.event.getClass() == eventClass) && (((ResponseEvent) this.event)
                    .getResponse().getStatusCode() == statusCode));
        }
    ...
    /**
     * Get the Method (Sip Method for SipRequestEvent or SipResponsEvent and
     * TIME_OUT String for TimeOut);
     * 
     * @return
     */
    ...
    public String getEventAsString() {
        return "Call Id: " + callId + ", Method: " + method + ", Class: "
